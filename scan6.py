...
def extract_text(image):
    ...
    # blurred = cv2.GaussianBlur(threshold_image, (0, 0), 3)
    # sharpened = cv2.addWeighted(image, 1.5, blurred, -0.5, 2)
    # sharpening_kernel = np.array([[0, -1, 0],
    #                              [-1, 5.5, -1],
    #                              [0, -1, 0]])
    # sharpened_image = cv2.filter2D(sharpened, -1, sharpening_kernel)
